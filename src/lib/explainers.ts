...
export const EXPLAIN: Record<string, Explainer> = {
  ...
  'transfer-window': { what: 'Two rings are the orbits, the arc between them is the cheapest transfer path, and the two dots must line up at the right angle for the trip to work.', how: 'Drag the slider to sweep the departure planet around its orbit. The cost and travel time never change — but only one phase angle actually arrives; that is the launch window.' },
  ...
  'atmosphere-column': { what: 'A slice of the sky stood on end, drawn to true height — each band is one layer of the atmosphere, the curve on the right is how fast the air pressure drops as you climb, and the pinned heights are places you already know.', how: 'Read bottom to top: ground level at the base, the edge of space at the top. The brown curve shows pressure falling away; the marks in the margin are landmark altitudes.' },
  'carbon-loop': { what: 'Each box is a place carbon is stored, sized by how much it holds; each arrow is a yearly flow between them, thicker and faster where more carbon moves. The brown mark shows the carbon that arrives but never leaves.', how: 'Follow the arrows around the loop — most nearly cancel out. Watch the brown residual on the atmosphere: that is the part that accumulates every year.' },
  'storm-track': { what: "One tropical cyclone's real path across the ocean, drawn from birth to breakup — the colour of the line is how strong it was at each point, from a weak depression to a top-category hurricane. The glowing point is its peak; the ringed points are where it hit land.", how: 'Drag to spin the globe and follow the track. Hover any point on the path for the exact wind speed, category, and date.' },
  ...
  'packet-trace': { what: 'The globe shows the actual path a request takes between cities; the bar below splits the round-trip time into the part physics forces (green) and the part everything else adds (pink).', how: 'Drag to spin the globe. Hover any bar segment to light up that leg of the journey and see how far it ran over the physical minimum.' },
  'queue-cliff': { what: 'The curve is how long the average request waits as the system fills up — it stays flat, then snaps vertical as load nears full capacity.', how: 'Drag the slider to raise the load. Watch the wait multiplier stay calm, then explode past about 90% — the readout gives the exact numbers.' },
  'chip-die': { what: 'A processor die as an exploded, to-scale floorplan: each tile is one functional block, and its size on screen equals its real share of the silicon.', how: 'Hover or tap a tile to lift it and read its area. The biggest tiles are where the chip actually spends its space.' },
  'moore-ladder': { what: 'How a technology’s count — like transistors per chip — explodes over the years, drawn on a ruler where each step up means twice as many, so the exponential becomes a straight climb.', how: 'Read up the ladder: every rung is double the last, and the straight line is the steady doubling. Hover any chip for its exact count.' },
  ...
  'city-grid': { what: "Each city is a compass wheel of 36 spokes; a spoke's length is how much of that city's streets run in that direction, so a planned grid draws a sharp cross and an old tangle draws a near-circle.", how: 'Compare the silhouettes: a sharp four-spoke cross is a surveyor grid, a full circle is organic streets. The order φ number sums it up.' },
  'altitude-oxygen': { what: "Height runs up the side; the column's width is how much oxygen the air still carries at that height. Each dot is a stop on the climb, and the teal tents mark the nights spent letting the body catch up.", how: 'Read up the column — the higher you go, the thinner it gets. The marked stops show where the trek pauses to acclimatize.' },
  'season-wheel': { what: "A destination's whole year as a tilted dial — twelve month-sectors, each with three stacked arcs for how good the weather is, how thick the crowds are, and how high the prices climb.", how: 'Slide through the months — the wheel lights each one and reads out its numbers. The marked arc is the sweet spot.' },
  'fare-terrain': { what: "Each ridge is one route's fare as departure nears — time runs left to right toward the flight, and higher ground means a pricier ticket. The shaded valley is the window where fares bottom out.", how: 'Follow a ridge from far-out on the left to the flight on the right; the low point in the shaded band is when to book.' },
  ...
  'bill-funnel': { what: 'The same bills counted again at every stage they had to clear. Bar length is how many were still alive there, and the faded segment is what was lost since the row above.', how: 'Press a stage to read what happened there. The widening white space to the right of the bars is the real output of the session.' },
  'channel-ternary': { what: 'A triangle, because the three shares must add to a hundred and only two are ever free. Each corner is all of one channel, the centre is an even split, and distance FROM a corner is how little that channel is used.', how: 'Press a dot or a table row for the three exact shares. A dot near an edge means the channel opposite that edge is barely used.' },
  'age-pyramid': { what: 'One row per age band, oldest at the top, bars running outward from a centre line with one group to each side. Read as counts for the size of each band, or as shares to compare the balance within unequal bands.', how: 'Press Counts or Share of band. Counts show where the population actually sits; shares show whether an imbalance is a generational effect working through, or roughly constant all the way up.' },
  'margin-bullets': { what: 'One row per subsystem, each drawn on its own scale because decibels and kilograms do not compare. The bar is what it has, the tick is what it needs, and a bar stopping short of its tick does not close.', how: 'Press a row for its signed margin against requirement, in that row\'s unit. Compare every bar to the ink tick on its own line, never to another row — the lengths across rows have nothing in common.' },
