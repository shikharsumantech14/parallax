...
- [Job-fit judgements](job-fit-judgements.md) — slot → job fits that held, and the arithmetic trap in "CONVERSATIONAL carries half"
- [check:prose gate quirks](check-prose-gate-quirks.md) — which fields it scores as prose, the three heuristic artifacts, the cheap exact fixes
- [AI tells that recur here](parallax-ai-tells.md) — the loud six are already clean; the unwritten connective is the real find
- [Protected fields vs the fix list](protected-fields-vs-fix-lists.md) — when a named fix lands in a do-not-touch field, and why unit conversions must be declared
