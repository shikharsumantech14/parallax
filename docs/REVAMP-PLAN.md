...
| **RD-08** | **Type harvest under RD-04:** adopt prose 16→18px (after a real measurement of the font binaries), instrument `h3` at 22px/700/−.024em in `--font-display`, 9.5px/600/.16em mono eyebrows, and a drop cap sized *"spans three lines"* in Fraunces — **the drop cap clause was ruled OUT of this decision's phase on 2026-09-05 and into Phase 7, where RD-12 already claims the issue furniture; RD-08 still specifies what it should look like, RD-12 owns when it ships.** **Reject** the fixed 68/40 headline steps — keep the fluid `clamp()`. The prototype has zero `@media` and zero `clamp`, so 68/40 is a two-width toggle, not a scale. |
...
| **RD-14** | **The section geometry on the floor plan (2026-09-23, air 2026-09-24).** A section has two widths: the measure (720, 69px inside each rule) and the breakout. `wide` keeps the text at the measure and lets the figure out 45px each side (810, 24px clear of each rule). `bleed`, `split` and `split-flip` are **aliases of `wide`** until a design pass re-derives them on the floor plan — the 1080 plate and the sticky two-column split were derived for the 980 single-column frame, crossed the rails RD-12 populated, and on the ISS issue covered the issue number and the contents list. `breath` is left-aligned. Nothing crosses the rails. `core/Section.astro` renders one markup for every layout. The size system's L does not exist inside the floor; re-deriving it is a design call this record does not pre-empt. |
| **RD-15** | **The render gate.** `npm run check:render` (`scripts/ui-probe.mjs`, headless Chrome) renders every non-draft issue as a signed-in reader at 1280 and 375 and fails on any element past the column, the honest phone overflow, clipped text, text on text, the ⤢ button on text or duplicate chrome; it writes a screenshot per section per width. It runs before any status flip and after any change under `src/components`, `src/styles` or `src/layouts`, and a commit hook refuses a rendering-relevant commit without a fresh clean run that covers it. Every textual gate passed while sixteen issues shipped visually broken; this is the gate that looks. |
...
|---|---|---|---|
| 1 | **6.1 finish** — ✅ the `Source ·` fold landed (`8eea66f`, `b0260b2`), the `EXPLAIN.how` flip is on (`bdbfea8`), the CANON/motion edits drafted as a marked DRAFT (`dc6a28c`), **`CANON.md` signed 2026-09-05** | operator: ~~signature on the canon edits~~ CANON done; `motion.md` waits on the `--t-page` retime call, which is the decision its DRAFT line is really holding | **met bar `motion.md`** — every viz card on every published issue carries the full shell; the canon drafts match the shipped CSS |
| 2 | **6.3 type harvest** (RD-08) — ✅ **complete 2026-09-05** | measure the font binaries first — the 16→18px step is conditional on it | **met**: prose **17.5px** (the measurement's answer, not RD-08's literal 18 — Schibsted's x-height is the larger), standfirst 20px, `h3` 22/700/−.024em on its one referent, 9.5/600/.16em eyebrows across 29 rules, 375px clean. The drop cap moved to Phase 7 by ruling and is NOT an exit condition here |
| 3 | **7 web pages** — the README's six: masthead, home, desk, issue, archive, about | RD-12 signed (3-column + gate rework in one commit) | see Phase 7 below, extended by v3 — **plus the three-line Fraunces drop cap, ruled here from 6.3 on 2026-09-05**, which lands with the rest of the furniture in the grid commit |
...
fiction and must not shape the empty state; the issue-page furniture per RD-12
(fact grid with the Voice cell, drop cap, meta strip) — RD-07 is struck and
carried its furniture list forward into RD-12; the **drop cap joins this list
by the 2026-09-05 ruling** that moved it out of 6.3, so every piece of
furniture now has one owner. **The 3-column grid
