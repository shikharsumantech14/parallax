...
3. **Phase 7 — six web pages** — masthead to the 768px spec; home; desk = the
   topic index reskinned (ruled); issue on the 3-column grid **with the gate
   rework in one commit** (RD-12), carrying the furniture: fact grid with the
   `Voice` cell, meta strip, primer strip, and **the three-line Fraunces drop
   cap, ruled here from 6.3 on 2026-09-05** — RD-08 still specifies what it
   looks like, RD-12 owns when it ships; `/archive` against the 10 real
   issues; About with the mark explained and the lore rows. `/subscribe`
   behind pricing.
4. **The brand** (RD-10) — the glyph outline proven on `mark.svg` first, *then*
   estimate; the swap list; a push busts favicon/OG caches.
5. **Phase 8 app** — manifest bridge first; eleven screens; compile-only here.
6. **Phase 5 — mobile legibility** (~4–6 days): generalise the
...
7. **Phase 3 Waves 2–4** — **reassess at the look's exit**: 21 kinds, ~22
   ...
   **with a standing corrections header — read it first; it overrides the
   ...
   defensible spend.
8. **Schema tightening** — make `source` required now the gap is 0. Its own
